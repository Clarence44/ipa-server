...
const itemInfo = (row, publicURL) =>
  Object.assign({}, row, {
    ...
    apk: `${config.publicURL || publicURL}/${row.identifier}/${row.id}/apk.apk`,
...
const find = (id, publicURL, platform) => {
  ...
  if (currentData.platform !== platform) {
    ...
    data.history = history.map((r) => {
      r.current = data.id === r.id;
      return r;
    });
    return data;
  }

  const row = itemInfo(currentData, publicURL);
  row.history = history;
